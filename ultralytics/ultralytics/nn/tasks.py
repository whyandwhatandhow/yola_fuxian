...
class DetectionModel(BaseModel):
    ...
    def loss(self, batch, preds=None):
        """Compute loss (detection loss + optional illumination-invariant loss)."""
        if getattr(self, "criterion", None) is None:
            self.criterion = self.init_criterion()
        ...
        ii_gain = float(getattr(getattr(self, "args", None), "ii", 0.0) or 0.0)
        ii_warmup = int(getattr(getattr(self, "args", None), "ii_warmup", 0) or 0)
        epoch = int(getattr(getattr(self, "args", None), "epoch", 0) or 0)
        if ii_warmup > 0 and epoch < ii_warmup:
            ii_gain = 0.0
        if ii_gain > 0:
            ii_losses = []
            for m in self.modules():
                v = getattr(m, "ii_loss", None)
                if v is not None:
                    ii_losses.append(v)
            ii = torch.stack(ii_losses).mean() if ii_losses else loss.new_zeros(())
            bs = batch["img"].shape[0]
            loss = torch.cat((loss, (ii * ii_gain * bs).unsqueeze(0)))
            loss_items = torch.cat((loss_items, (ii.detach() * ii_gain).unsqueeze(0)))
